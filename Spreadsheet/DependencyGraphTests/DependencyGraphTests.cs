...
namespace DevelopmentTests
{
    ...
    public class DependencyGraphTest
    {
        ...
        public void AddCycleTest()
        {
            ...
            t.AddDependency("x", "x");
            t.AddDependency("y", "y");
            HashSet<string> temp = (HashSet<string>)t.GetDependents("x");
            Assert.IsTrue(temp.Contains("x"));
            temp = (HashSet<string>)t.GetDependents("y");
            Assert.IsTrue(temp.Contains("y"));
        }

        [TestMethod()]
        public void RemoveNonContainedDependency()
        {
            DependencyGraph t = new DependencyGraph();
            t.AddDependency("x", "z");
            t.AddDependency("y", "x");
        ...
        [TestMethod()]
        public void AddThenReplaceDependency()
        {
            DependencyGraph t = new DependencyGraph();
            t.AddDependency("x", "y");
            t.AddDependency("x", "z");
            t.ReplaceDependents("x", new HashSet<string>() { "a" });
            HashSet<string> temp = (HashSet<string>)t.GetDependents("x");
            Assert.IsTrue(temp.Count == 1 && temp.Contains("a"));
        }

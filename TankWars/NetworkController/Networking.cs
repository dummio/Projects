...
{
    /// <summary>
    /// A networking library that wraps the C# networking details in cleaner methods. 
    /// </summary>
    public static class Networking
    {
        ...
        {
            Tuple<Action<SocketState>, TcpListener> asyncInfo = (Tuple<Action<SocketState>, TcpListener>)ar.AsyncState;

            TcpListener listener = (TcpListener)asyncInfo.Item2;

            Action<SocketState> toCall = (Action<SocketState>)asyncInfo.Item1;

            try
            {
                Socket client = listener.EndAcceptSocket(ar);

                SocketState state = new SocketState(toCall, client);

                state.OnNetworkAction(state);

                listener.BeginAcceptSocket(AcceptNewClient, asyncInfo);
            }
        ...
        public static void ConnectToServer(Action<SocketState> toCall, string hostName, int port)
        {
            ...
            try
            {
                ...
                if (!foundIPV4)
                {
                    ...
                    ErrorState.ErrorMessage = "No IPV4 addresses found.";
            ...
            try
            {
                ...
                // Attempts to connect to the server for 3 seconds, and returns true if it succeeds
                bool success = result.AsyncWaitHandle.WaitOne(3000, true);

                // If the server attempt connect failed, shut down the connection
                if (!success)
                    socket.Close();
            }
            catch (Exception e)
        ...
        private static void ReceiveCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                // If 0 bytes are received, the connection is closed and is handled like an error
                if (numBytes > 0)
                {
                    lock (state)
                    {
                ...
                else
                {
                    ...
                    ErrorState.ErrorMessage = "No bytes received, socket is closed.";
        ...
        private static void SendAndCloseCallback(IAsyncResult ar)
        {
            ...
            // If an error is caught, do nothing
            catch (Exception)
            {
            }
